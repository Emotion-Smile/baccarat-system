...
test('verify correct identification and properties of winning tickets in dragon tiger game', function () {
    ...
    $winningTickets = (new BaccaratGameGetWinningTicketsAction())($game);
    assertDatabaseCount(BaccaratTicket::class, 11);
...
test('ensures correct query of winning tickets in Dragon Tiger game', function () {
    ...
    $winningTickets = BaccaratTicket::query()
        ->onlyWinningTickets(game: $game)
        ->get();

    $winningTicketsFromCollection = $winningTickets->filter(fn(BaccaratTicket $ticket) => $ticket->isWinning());
