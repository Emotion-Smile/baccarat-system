...
const dragon = computed(()=>gameStore.dragon);
const dragonColors = computed(()=>gameStore.dragonColors);
...
<template>
    <div
        ...
        :style="[`background-image: url(${asset('dragon-tiger/images/Dragon.png')})`]"
    >
        <div class="flex h-full flex-col gap-1">
            <SquaredBox
                class="relative h-1/2 overflow-hidden border-2 bg-dragon-dragon/80 py-2 pr-12 z-0"
                :class="[gameStore.checkMainResult('dragon') ? 'border-yellow-300 shadow shadow-yellow-300': 'border-dragon-red', notAllowedBet? 'disabled pointer-events-none': 'cursor-pointer']"
                @click="onBet(dragon.type)"
            >
                <template #title>
                ...
                <template #coin>
                    <div class="absolute left-0 bottom-0">
                        ...
                        >
                            <template v-if="dragon?.totalBetAmount">
                                <img
                                    class="absolute inset-0 z-[0] w-full"
                                    :src="asset(`dragon-tiger/images/${gameStore.getChip(dragon.totalBetAmount)}.png`)"
                                    alt="bet"
                                >
                ...
                <template #card>
                    <img
                        v-if="gameStore.checkMainResult('dragon')"
                        class="absolute left-0.5 top-0 h-8"
                        :src="asset('dragon-tiger/images/win.svg')"
                    >
                </template>
            </SquaredBox>
            <div
                class="flex flex-1 bg-dragon-dragon/80"
            >
                <SquaredBox
                    v-for="(color, index) in dragonColors"
                    :key="index"
                    class="w-1/2 relative border-dragon-red"
                    :class="{
                ...
                >
                    <template #small-box-title>
                        <div
                            ...
                            class="mx-auto flex-row h-6 w-full items-center justify-center px-1 py-0.5"
                        >
                    ...
                    <template #coin>
                        <div class="absolute left-0 md:bottom-0">
                            <div
                                class="relative flex h-14 w-14 items-center justify-center"
                            >
