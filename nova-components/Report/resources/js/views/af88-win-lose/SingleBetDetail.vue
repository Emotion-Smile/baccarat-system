...
export default {
    metaInfo() {
        return {
            title: 'AF88 Single Bet Detail Report',
        };
    },
    ...
    methods: {
        setData({ report }) {
            ...
            // this.previousUserId = report.previousUserId;
        },
    ...
    computed: {
        requestUrl() {
            return `/nova-vendor/report/af88/win-lose/single-bet/detail/${this.username}`;
        },
...
<template>
    <div class="relative">
        <div class="flex item-center mb-3">
            ...
            <heading
                :level="1"
                v-html="`AF88 Single Bet Detail Report`"
            />
        </div>
                        ...
                        >
                            ...
                            <td class="border-r">
                                <span class="whitespace-no-wrap">
                                    <h5 class="text-primary">
                                        {{ ticket.event.name }} ({{ ticket.sport.name }})
                                    </h5>

                                    <h5>
                                       {{ ticket.match.home }} -VS- {{ ticket.match.away }} 
                                       <span v-if="ticket.isHalfTime" class="font-light">(First Half)</span>
                                    </h5>

                                    <h5 class="font-light">{{ ticket.match.startedAt }}</h5>
                                </span>
                            </td>
                            ...
                            <td class="border-r">
                                <span class="whitespace-no-wrap">
                                    <h5>
                                        <span 
                                            v-if="ticket.isHdpBet" 
                                            :class="{ 
                                                'text-danger': ticket.match.striker === ticket.betOnType 
                                            }"
                                        >
                                            {{ ticket.betOnType === 1 ? ticket.match.home : ticket.match.away }}
                                        </span>

                                        <span 
                                            v-else 
                                            :class="{ 
                                                'text-danger' : ticket.betOnType === 3, 
                                                'text-primary' : ticket.betOnType === 4
                                            }"
                                        >
                                            {{ ticket.betOnType === 3 ? '(Over)' : '(Under)' }}
                                        </span>
                                    </h5>

                                    <h5>
                                        {{ ticket.hdp }} @ {{ ticket.odds }}
                                    </h5>
                                </span>
                            </td>
                            ...
                            <td class="text-center border-r">
                                <span class="whitespace-no-wrap">
                                    <h5>
                                        HT: {{ ticket.result.halfTime }}
                                    </h5>

                                    <h5 v-if="!ticket.isHalfTime">
                                        FT: {{ ticket.result.fullTime }}
                                    </h5>
                                </span>
                            </td>
