...
class Transaction extends BaseTransaction
{
    ...
    {
        return $this->belongsTo(Transfer::class, 'id', 'deposit_id');
    }

    public function withdraw(): BelongsTo
    {
        return $this->belongsTo(Transfer::class, 'id', 'withdraw_id');
    }
    ...
    public function scopeFilterByPeriod($query, string $column)
    {
        ...
        return $query->when($date, function (Builder $query, string $date) use ($column) {
            ...
            if ($date === Period::LAST_MONTH) {
                $query->whereDate($column, '>=',  now()->subMonth()->startOfMonth()->format('Y-m-d'))
                    ->whereDate($column, '<=', now()->subMonth()->endOfMonth()->format('Y-m-d'));
            }
        });
    }
