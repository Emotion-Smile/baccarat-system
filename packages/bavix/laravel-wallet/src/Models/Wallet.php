...
use Bavix\Wallet\Exceptions\NotAllowComputeAvailableBalance;
...
class Wallet extends Model implements Customer, WalletFloat, Confirmable, Exchangeable
{
    ...
    /**
     * @return float|int
     * @throws NotAllowComputeAvailableBalance
     */
    public function getAvailableBalance()
    {
        $wallet = Wallet::select(['balance'])->find($this->getKey());

        if (!$wallet) {
            throw new NotAllowComputeAvailableBalance();
        }

        return (int)$wallet->getOriginalBalance();
